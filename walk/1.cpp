...
void dec()
{
	for (int i=1;i<=Len;i++) T[i]=s[i]-'0';
	T[1]--;
	int now=1;
	while (T[now]<0) { T[now]+=2;T[now+1]--;now++;}
}
int calc()
{
	int sum=0;
	for (int i=Len;i>=1;i--) sum=sum<<1|T[i];
	return sum;
}
int main()
{
